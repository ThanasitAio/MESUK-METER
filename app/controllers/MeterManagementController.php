...
public function saveMeter() {
    ...
    try {
        ...
        error_log("saveMeter: pcode=$pcode, month=$month, year=$year, electricity=$electricity, water=$water, garbage=$garbage, common_area=$common_area");
        
        // ตัวแปรเก็บผลลัพธ์
        $allSuccess = true;
        $errors = array();
        
        // บันทึกข้อมูลค่าไฟ (บันทึกแม้ค่าเป็น 0)
        ...
        $electricityResult = $this->meterModel->saveOrUpdateMeter($electricityData);
        ...
        // บันทึกข้อมูลค่าน้ำ (บันทึกแม้ค่าเป็น 0)
        $waterData = array(
            'pcode' => $pcode,
            'month' => $month,
            'year' => $year,
        ...
        $waterResult = $this->meterModel->saveOrUpdateMeter($waterData);
        ...
        // บันทึกค่าขยะ (บันทึกแม้ค่าเป็น 0)
        $garbageResult = $this->saveOtherCost($pcode, $month, $year, 'ค่าขยะ', $garbage);
        ...
        // บันทึกค่าส่วนกลาง (บันทึกแม้ค่าเป็น 0)
        $commonAreaResult = $this->saveOtherCost($pcode, $month, $year, 'ค่าส่วนกลาง', $common_area);
